...
WHERE
	employee.branchID = ? AND
	todo.startTime >= CURDATE() - 2 AND
...
if($stmt = mysqli_prepare($link, $sql)){
    ...
    if(mysqli_stmt_execute($stmt)){
        ...
        /* bind result variables */
        mysqli_stmt_bind_result($stmt, $bTodoID,$bTodoStartDate, $bTodoEndDate, $bTodoStartTime, $bTodoEndTime, $bTodoContent, $bTodoStatus, $btodoEmpID, $bEmpName, $bLate);
        ...
        while (mysqli_stmt_fetch($stmt)) {
            ...
            $todoEmpID[$NoOfTodo] = $btodoEmpID;
            $todoEmpName[$NoOfTodo]=$bEmpName;
...
//Check if less than Todo Count => set first page result
if ($NoOfTodo <= $result_per_page) {
    $result_per_page = $NoOfTodo;
};

$start_from = 0;
//check GET method for page load
if (isset($_GET["page"])) { 
    $page  = $_GET["page"]; 
} 
else { $page=1; };
...
if($stmt = mysqli_prepare($link, $sql)){
    ...
    if(mysqli_stmt_execute($stmt)){
        ...
        /* bind result variables */
        mysqli_stmt_bind_result($stmt, $bEmpID, $bEmpFullName, $bEmpStatus );
        $NoOfEmp = 0;
        
        while (mysqli_stmt_fetch($stmt)) {
            $empIDList[$NoOfEmp] = $bEmpID;
            $empFullNameList[$NoOfEmp] = $bEmpFullName;
            $empStatusList[$NoOfEmp] = $bEmpStatus;
  ...
  <nav class="main-header navbar navbar-expand navbar-warning navbar-light">
    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item d-none d-sm-inline-block">
        ...
        //echo "Resule per Page" . $result_per_page . "<br>";
        //echo "No Of Todo" . $NoOfTodo . "<br>";
        ?></a>
      </li>
            ...
            <ul class='nav nav-treeview'>
              <li class='nav-item'>
                ...
                "; 
                  ...
                  echo "
           <li class='nav-item'>
            <a href='branchWorkManage.php' class='nav-link active'>
    ...
    <section class="content">
      <div class="container-fluid">
        ...
        <div class="row">
            ...
            <!-- Row display Late Request -->
            <?php 
            if ($NoOfRequestLate > 0) {
                echo "
                    <div class = 'row'> 
                    	<div class = 'col-lg-12' >
                    		<div class='card'>
                    			...
                    			<div class='card-header'>
                                        <h3 class='card-title'>Phê duyệt đi trễ</h3>
                    				...
                    				<table class='table table-hover text-nowrap'>
                                          ...
                                          for ($i=0; $i<$NoOfTodo; $i++) {
                                              if ($todoLate[$i] == 'request') {
                                                  ...
                                                  <td>". $todoContent[$i] . "</td>
                                                  <td>". $todoEmpName[$i] . "</td>
                                                  <td>". $todoStartTime[$i] . "</td>
                                                  <td>". $todoEndTime[$i] . "</td>
                                                  <td>". " 
                                                      <div class='btn-group'>
                                                        <button id='" . $todoID[$i] . "' type='button' class='accept_late btn btn-success'><i class='fas fa-user-check'></i></button>
                                                        <button id='" . $todoID[$i] . "' type='button' class='denied_late btn btn-danger'><i class='fas fa-user-times'></i></button>
                                                      </div>  
                                                  " ."</td>
                                                  </tr>";
                                              }
            ...
            <div class="row">
            	<div class = "col-lg-12">
                            ...
                            <div class="card">
                              <div class="card-header">
                                <h3 class="card-title">Thống kê công việc theo nhân viên</h3>
                                ...
                                <table class="table table-hover text-nowrap">
                                  ...
                                  for ($i=0; $i<$NoOfEmp; $i++) {
                                      ...
                                      for ($n=0; $n<$NoOfTodo; $n++) {
                                          if($todoEmpID[$n] == $empIDList[$i] && $todoStatus[$n] == "Finished"){
                                              $countTodoFinished++;
                                          }
                                      }
                                      ...
                                      $countTodo = 0;
                                      for ($n=0; $n<$NoOfTodo; $n++) {
                                          if($todoEmpID[$n] == $empIDList[$i]){
                                              $countTodo++;
                                          }
                                      }
                                      $processingTodo = $countTodo - $countTodoFinished;
                                      
                                      echo "
                                      <tr>
                                      <td>". $stt . " </td>
                                      <td>". $empFullNameList[$i] . "</td>
                                      <td>". $countTodoFinished . "</td>
                                      <td>". $processingTodo."</td>
                                      </tr>";
                                  }
                                  ?>  
            ...
            <div class="card">
              <div class="card-header">
                ...
                <div class="card-tools">
                  <ul class="pagination pagination-sm">
                    ...
                    if ($NoOfTodo > $result_per_page) {
                        $pageTodo = intdiv($NoOfTodo, $result_per_page) + 1;
                        
                    } else $pageTodo = 1;
                    
                    for ($i = 1 ; $i <= $pageTodo; $i ++) {
                        echo "<li class='page-item'><a href='branchWorkManage.php?page=" . $i . "' class='page-link'>";
                        echo $i;
                        echo "</a></li>";
                    }
                    ...
                    $recordLoad = $result_per_page;
                    if ($page == $pageTodo) {
                        $recordLoad = $NoOfTodo - (($pageTodo - 1) * $result_per_page);
                    } else $recordLoad = $result_per_page;
                    
                    //echo "Record Load : ". $recordLoad . "<br>";
                    //echo "Current Page : ". $page . "<br>";
                    //echo "Total Page : ". $pageTodo . "<br>";
                    //echo "Start Record : ". $start_from . "<br>";
              ...
              <!-- /.card-header -->
              <form id="form_todoList" action="db/updateTodoStatus_Branch.php" method="post">
              <div class="card-body">
                <ul class="todo-list" data-widget="todo-list">
                  <?php 
                 
                  //New array for Todo Collection per page => POST to new Page
                  $NoOftodo_perPage = 0;
                                   
                  for ($i=$start_from; $i<($start_from + $recordLoad); $i++) {
                      echo "
                <li>
                    <!-- drag handle -->
                    <span class='handle'>
                    ...
                    <small class='badge badge-danger'><i class='fas fa-user-tie'></i>" ." " . $todoEmpName[$i]. "</small>";
                    ...
                    $todoID_perPage[$NoOftodo_perPage] = $todoID[$i];
                    $todoStatus_perPage[$NoOftodo_perPage] = $todoStatus[$i];
                    $NoOftodo_perPage ++;
                  }
                  
                  $_SESSION["todoID_perPage"] = $todoID_perPage;
                  $_SESSION["todoStatus_perPage"] = $todoStatus_perPage;
            ...
            <section class="col-lg-5 connectedSortable">
				...
				<div class="row">
				<div class="col-lg-12">
    			 <form id="addTodo" action="db/addTodoBranch.php" method="post">
                      ...
                      <div class="card-body">
        				...
        				<div class="row">
        					<div class="col-lg-12">
                              <!-- textarea -->
                              <div class="form-group">
                                <label>Phân giao nhân viên</label>
                                <select multiple id="empID" name="empID[]" class="form-control">
                                	
                                  <?php 
                                  for ($i=0; $i < $NoOfEmp; $i++) {
                                      echo "<option value='" . $empIDList[$i] . "'>" . $empFullNameList[$i] . "</option>";
                                  }
  	...
  	<div class="modal-dialog">  
           <div class="modal-content">  
                ...
                <div class="modal-body">  
                     <form name="edit_form" action="db/updateTodoFullContent_Branch.php" method="post">  
                          <div class = "row">
                        	<div class = "col-lg-6">
                        		<div class="bootstrap-timepicker">
                                  <div class="form-group">
                		...
                		<div class="row">
        					<div class="col-lg-12">
                              ...
                              <div class="form-group">
                                <label>Phân giao nhân viên</label>
                                <select id="empID_Update" name="empID_Update" class="form-control">
...
<script>  
 $(document).ready(function(){  
   ...
   // Delete 
	...
	// Accept Late
	  $('.accept_late').click(function(){
		 // toastr.success('Phê duyệt Đi trễ thành công.','Thông báo')
			    
	    // Todo id
	    var todoID = $(this).attr('id');
	 
	    // Confirm box
	    bootbox.confirm("Bạn có muốn Duyệt đi trễ công việc này không ?", function(result) {
	 
	       if(result){
	         // AJAX Request
	         $.ajax({
	           url: 'db/acceptLate.php',
	           type: 'POST',
	           data: {todoID: todoID},
	           success: function(response){
				//bootbox.alert(response);
	             ...
	             if(response){
					 ...
					 toastr.error('Lỗi phê duyệt. Vui lòng liên hệ IT để được hỗ trợ')
		     	 }
	...
	// Denied Late
	  $('.denied_late').click(function(){
	    ...
	    // Confirm box
	    bootbox.confirm("Bạn có muốn Từ chối đi trễ công việc này không ?", function(result) {
	 
	       if(result){
	         // AJAX Request
	         $.ajax({
	           url: 'db/deniedLate.php',
	           type: 'POST',
	           data: {todoID: todoID},
	           success: function(response){
				//bootbox.alert(response);
	             // Refresh page
	             if(response){
	            	 toastr.success('Từ chối Đi trễ thành công.','Thông báo',{
                         "showDuration":"1000",
                         "timeOut" : "8000",
