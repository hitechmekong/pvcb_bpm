...
$isGrandPermissionError = false;
$isGrandPermissionSuccess = false;

if (isset($_SESSION["grandPermissionStatus"])){
  $grandPermissionStatus = $_SESSION["grandPermissionStatus"];
} else $grandPermissionStatus = "";

$grandPermissionResponse = "";

if (isset($_SESSION["ResponseString"]) && ($grandPermissionStatus == "Error")) {
    $isGrandPermissionError = true;
    $grandPermissionResponse = $_SESSION["ResponseString"];
} else if (isset($_SESSION["ResponseString"]) && ($grandPermissionStatus == "Success")) {
    $isGrandPermissionSuccess = true;
    $grandPermissionResponse = $_SESSION["ResponseString"];
}

$NoOfEmpByRole = 0;
//check role variable is set => select All employee with that role
if (isset($_GET["r"]) && $_GET["r"] > 0) {
  $selectedRoleID = $_GET["r"];
  ...
  if($stmt = mysqli_prepare($link, $sql)){
      ...
      if(mysqli_stmt_execute($stmt)){
          ...
          mysqli_stmt_bind_result($stmt,$empID, $bUsername, $empName);
          while (mysqli_stmt_fetch($stmt)) {
              $empIDByRole[$NoOfEmpByRole] = $empID;
              $empUserName[$NoOfEmpByRole] = $bUsername;
              $empNameByRole[$NoOfEmpByRole] = $empName;
              $NoOfEmpByRole++;
          }
...
if($stmt = mysqli_prepare($link, $sql)){
    ...
    if(mysqli_stmt_execute($stmt)){
        ...
        /* bind result variables */
        mysqli_stmt_bind_result($stmt, $bRoleID, $bRoleName);
        //$NoOfTodo = 0;
        
        while (mysqli_stmt_fetch($stmt)) {
            $roleIDList[$NoOfRole] = $bRoleID;
            $roleNameList[$NoOfRole] = $bRoleName;
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          for ($i=0; $i<$NoOfFunction; $i++) {
            ...
            <ul class='nav nav-treeview'>
              <li class='nav-item'>
                <a href='empPermissionGrand.php' class='nav-link active'>
                  <i class='far fa-circle nav-icon'></i>
  ...
  <div class="content-wrapper" >
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Phân quyền truy cập cho nhân viên</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Quản lý nhân viên</a></li>
              <li class="breadcrumb-item active">Phân quyền nhân viên</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        ...
        <!-- Change pass Form -->
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="card-title">Phân quyền theo chức danh</h3>
              </div>
              <form id="grandPermission" action="db/grandPermissionDB.php" method="post">
              <div class="card-body">
				...
				<div class="row">
					<div class="col-lg-12">
                      ...
                      <div class="form-group">
                        <label>Chức danh cần phân quyền</label>
                        <select class="form-control" id="sel_roleID" name="sel_roleID">
                          <option value=0>Vui lòng chọn chức danh</option>
                          <?php 
                          for ($i = 4; $i< $NoOfRole ; $i++) {
                              echo "<option value=" . $roleIDList[$i];
                              if ($selectedRoleID == $i + 1) {
                                echo " selected";
                              }
                              echo ">" . $roleNameList[$i] . "</option>";
                          };
                ...
                <!-- textarea -->
                <div class="form-group">
                  <label>Lựa chọn nhân viên cần phân quyền</label>
                  <select multiple id="empList" name="empUsers[]" class="form-control">
									<?php 
                    if ($NoOfEmpByRole > 0) {
                      for ($i=0; $i<$NoOfEmpByRole ; $i++) {
                        echo "<option value=" . $empUserName[$i] . ">" . $empNameByRole[$i] . "</option>";
                      }
                    } else echo "<option>Không có dữ liệu, hãy chọn chức danh khác</option>"
        ...
        <div class="row">
            <div class="col-lg-12">
                <!-- textarea -->
                <div class="form-group">
                  <label>Cấp phân quyền</label>
                   <div class="select2-blue">
                    <select id="funcList" name="funcIDs[]" class="select2" multiple="multiple" data-placeholder="Lựa chọn theo danh sách" data-dropdown-css-class="select2-blue" style="width: 100%;">
                      <?php 
                        if ($NoOfEmpByRole > 0) {
                          if($selectedRoleID == 5 || $selectedRoleID == 6) {
                            ...
                            echo '<option value=10>Quản lý nợ xấu</option>';
                            echo '<option value=11>Đào tạo trực tuyến</option>';
                         } else if ($selectedRoleID == 4 || $selectedRoleID == 8) {
                            echo '<option value=2>Bảng thống kê</option>';
                            echo '<option value=4>Quản lý KPI</option>';
                            echo '<option value=6>Quản lý khách hàng</option>';
                            echo '<option value=7>Quản lý hồ sơ</option>';
                            echo '<option value=9>Quản lý công việc</option>';
                            echo '<option value=10>Quản lý nợ xấu</option>';
                            ...
                            echo '<option value=1>Bảng thống kê</option>';
                            echo '<option value=12>Quản lý KPI</option>';
        ...
        if ($isGrandPermissionError) {
            echo "
    
            <div class='row'>
              <div class='col-lg-12'>
                          <span class='help-block' style='color: red'> " . $grandPermissionResponse . "</span>
          
                        </div>
            </div>
          
                    ";
            $_SESSION["grandPermissionResponse"] = "";
            $_SESSION["grandPermissionStatus"] = "";
        }
        
        if ($isGrandPermissionSuccess) {
            echo "
    
            <div class='row'>
              <div class='col-lg-12'>
                          <span class='help-block' style='color: blue'> " . $grandPermissionResponse . "</span>
        ...
        ?>
				...
				<div class="card-footer">
                  <button id="change_pass" name="change_pass" type="submit" class="btn btn-primary"><i class="fas fa-sync-alt"></i> Cấp quyền</button>
              </div>
...
$(document).ready(function(){
    $('#sel_roleID').change(function(){
        var roleID = $(this).val();
        var url = 'empPermissionGrand.php?r=' + roleID;
